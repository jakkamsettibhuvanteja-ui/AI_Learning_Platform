...
st.caption(f"Personalized Circullum Roadmap generated for {user_name} according to the Current Marketing Standards.")
st.markdown("---")
...
if "ai_data" not in st.session_state or st.session_state.ai_data is None:
    ...
    st.write("Skip the basic tutorials. Focus on these complex core competencies to unlock elite tier profiles:")
    ...
    if isinstance(better_list, list) and len(better_list) > 0:
        ...
        st.info("💡 The AI did not return premium path metrics for this specific stack combination. Try adjusting your sidebar Parameters.")

    st.divider()
    st.header("💾 Export your Personalized Circullum")
    ...
    st.download_button(
        ...
        data=download_text,
        file_name=f"{user_name}_{target_role.replace(' ',',')}_Roadmap.txt",
        mime="text/plain",
        use_container_width = True
    )
    st.success("🎯 Project Execution Complete. Your Custom AI Learning engine is fully synchronchronized with 2026 Industry Frameworks.")
